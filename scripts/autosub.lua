-- modified by mpv_slim: config start
local subliminal = 'subliminal'
local languages = { { 'Chinese', 'zh', 'chi' }, { 'English', 'en', 'eng' } }
local logins = {
...
local includes = {}
-- modified by mpv_slim: config end
...
-- modified by mpv_slim: bindings start
mp.add_key_binding('b', 'download_subs', download_subs)
-- modified by mpv_slim: download-subs start
mp.register_script_message('download-subs', function()
    input.select({
        items = {
            "Chinese (中文)",
            "English",
        ...
        submit = function(id)
            if id == 1 then download_subs(languages[1])
            elseif id == 2 then download_subs(languages[2]) end
        end
    })
end)
-- modified by mpv_slim: download-subs end
mp.register_script_message('download-subs2', function() download_subs2() end)
mp.register_event('file-loaded', control_downloads)
-- modified by mpv_slim: bindings end